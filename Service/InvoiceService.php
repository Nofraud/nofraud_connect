 ...
 * This class is responsible for creating and capturing invoices for orders that have been
 *  approved by NoFraud.
 */
class InvoiceService
{
    ...
    public function createInvoice($order, $isCron = false): void
    {
        try {
            if (!$this->configHelper->authCaptureEnabled()) {
                $this->dataHelper->addDebugToLog(__('Auth capture is disabled. Skipping invoice creation for order #%1', $order->getIncrementId()));
                return;
            }

            $this->dataHelper->addDebugToLog(__('createInvoice invoked for order #%1', $order->getIncrementId()));

            // Fetch full order object if invoked from cron
            if ($isCron) {
                $this->dataHelper->addDebugToLog(__('Invoice service invoked from cron. Fetching full order object for order #%1', $order->getIncrementId()));
                $order = $this->orderRepository->get($order->getId());
            }

            // Check if order has already been invoiced
            if ($order->getInvoiceCollection()->count() > 0) {
                $this->dataHelper->addDebugToLog(__('Error creating invoice for order #%1: Order has already been invoiced.', $order->getIncrementId()));
            ...
            if (!$order->canInvoice()) {
                ...
                $this->dataHelper->addDebugToLog(__('Unable to create invoice for order #%1: %2', $order->getIncrementId(), implode(", ", $reasons)));
            ...
            $this->dataHelper->addErrorToLog(__('Error creating invoice for order #%1: ' . $e->getMessage(), $order->getIncrementId()));
            ...
            if ($order->canHold()) {
                $this->dataHelper->addDebugToLog(data: __('Placing order #%1 on hold due to invoice creation error.', $order->getIncrementId()));
                $order->hold()->save();
            }
    ...
    private function handleCreateInvoice($order): Invoice
    {
        ...
        $this->dataHelper->addDebugToLog(__('Invoice #%1 created for order #%2', $invoice->getIncrementId(), $order->getIncrementId()));
